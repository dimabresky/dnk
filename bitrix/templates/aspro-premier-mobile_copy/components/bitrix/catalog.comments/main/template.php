		...
		$arData["VK"] = array(
			...
			"CONTENT" => '
				...
				<script type="text/javascript">
					BX.load([\'//vk.ru/js/api/openapi.js\'], function(){
						if (!!window.VK)
						{
							VK.init({
...
?><div id="<? echo $templateData['TABS_ID']; ?>" class="bx-catalog-tab-section-container tabs arrow_scroll swipeignore">
	<div class="tabs_wrap relative">
		<ul class="bx-catalog-tab-list1 nav nav-tabs mobile-offset" style="left: 0;"><?
			foreach ($arData as $tabId => $arTab)
			{
				if (isset($arTab["NAME"]) && isset($arTab["CONTENT"]))
				{
					...
					?>
					<li id="<?=$id?>" class="font_15 <?=$tabId;?>">
						<a href="#<?=$id;?>_cont" data-toggle="tab"><?=$arTab["NAME"]?></a>
					</li>
					<?
