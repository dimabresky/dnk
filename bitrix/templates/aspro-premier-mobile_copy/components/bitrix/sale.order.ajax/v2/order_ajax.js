...
(function () {
  ...
  BX.Sale.OrderAjaxComponent = {
    ...
    getFirstRequiredUncheckedConsentId: function () {
      ...
      return uncheckedConsent ? uncheckedConsent.code : null;
    },

    refusedByConsent: function (item) {
      BX.Sale.OrderAjaxComponent.consentQueue = false;
      if (item.config.code) {
        var consent = this.options.userConsents.find((consent) => consent.code === item.config.code);
        if (consent) {
          consent.checked = "N";
          this.validateConsent(item, false);
        }
      }
      this.disallowOrderSave();
    },
    ...
    saveByConsent: function (item) {
      if (item.config.code) {
        ...
        if (consent) {
          consent.checked = "Y";
          this.validateConsent(item, true);
        }
      }

      var requiredUncheckedConsentId = this.getFirstRequiredUncheckedConsentId();
      if (requiredUncheckedConsentId && BX.Sale.OrderAjaxComponent.consentQueue) {
    ...
    editRegistrationForm: function (authContent) {
      ...
      if (showSmsConfirm) {
        ...
        if (this.result.AUTH.captcha_registration == "Y") {
          authFormNodes.push(
            BX.create("DIV", {
              props: { className: "bx-authform-formgroup-container bx-captcha-container" },
              children: [
                BX.create("DIV", {
                  ...
                  children: [
                    ...
                    BX.create("SPAN", {
                      props: { className: "bx-authform-label" },
                      text: BX.message("CAPTCHA_REGF_PROMT"),
                    }),
                  ],
                }),
                BX.create("DIV", {
        ...
        if (
          ...
          appAspro?.userConsent?.register?.license_register_block?.HTML
        ) {
          const nodeRegisterLicense = BX.create("div", {
            props: {
              className: "bx-authform-formgroup-container license_block",
            },
            children: [appAspro.userConsent.register?.license_register_block.HTML],
          });

          const nodeLicenseText = nodeRegisterLicense.querySelector(".main-user-consent-request-announce-link");
          if (nodeLicenseText) {
            nodeLicenseText.innerText = nodeLicenseText.innerText.replace(
              BX.message("ORDER_DEFAULT_BUTTON_CONSENT"),
              BX.message("STOF_REGISTER")
            );
          }
      ...
      this.licensesConditions.forEach(($node) => {
        if (!$node.querySelector("input").checked) {
          if (
            ...
            && $node.querySelector('[data-bx-user-consent]')
          ) {
            const requiredUncheckedConsentId = this.getFirstRequiredUncheckedConsentId();
            if (requiredUncheckedConsentId && !isConsentInProcess) {
              BX.onCustomEvent(`bx-soa-order-save-${requiredUncheckedConsentId}`);
              isConsentInProcess = true;
            }
          }
    ...
    editTotalBlock: function () {
      ...
      if (!this.result.SHOW_AUTH) {
        ...
        const getCheckboxContainer = (htmlText = "") => {
          if (!htmlText || !String(htmlText).trim()) {
            return null;
          }
            ...
            this.totalBlockNode.querySelector(".bx-soa-total-wrapper").appendChild(
              BX.create("DIV", {
                ...
                children: [
                  BX.create("div", {
                    props: {
                      className: "bx-soa-cart-conditions-text font_xs flexbox gap gap--12",
                    },
    ...
    initUserConsent: function () {
      if (typeof this.options.userConsents !== 'undefined') {
        return;
      }
      ...
      BX.ready(
        BX.delegate(function () {
          if (this.options.userConsents) {
            if (BX.UserConsent) {
              let controls = BX.UserConsent.getItems();
            ...
            const control = BX.UserConsent && BX.UserConsent.load(this.orderBlockNode);
            if (control) {
