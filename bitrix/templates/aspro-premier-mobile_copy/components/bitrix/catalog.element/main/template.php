...
if ($bShowSelectOffer) {
    ...
    if ($arParams['SET_SKU_TITLE'] !== 'N') {
        ...
        $templateData['OFFER_INFO'] = [
            'NAME' => $arCurrentOffer['NAME'],
            'IPROPERTY_VALUES' => !empty($arCurrentOffer['IPROPERTY_VALUES']) ? $arCurrentOffer['IPROPERTY_VALUES'] : [],
        ];
    }
...
<?endif; ?>
<?// props content?>
<?$templateData['SHOW_CHARACTERISTICS'] = false; ?>
                ...
                <div class="grid-list grid-list--items gap gap--24">
                    ...
                    <?ob_start(); ?>
                        ...
                        <?else:?>
                            ...
                            <div class="catalog-detail__cart js-replace-btns js-config-btns<?=$arBasketConfig['HTML'] ? '' : ' hidden'; ?> border-bottom pb pb--24" data-btn-config='<?=str_replace('\'', '"', CUtil::PhpToJSObject($arBtnConfig, false, true)); ?>'>
                                <?=$arBasketConfig['HTML']; ?>
                            </div>
                        <?endif; ?>
                        
                    <?$btnHtml = trim(ob_get_clean()); ?>
...
<?$this->SetViewTarget('catalog_element_buy_button'); ?>
    <?if ($arBtnConfig):?>
        <div class="bottom-icons-panel__content bottom-icons-panel__content--buy-buttons line-block--column">
            <?php
            $arBasketConfig = TSolution\Product\Basket::getOptions(array_merge(
                $arBtnConfig,
                [
        ...
        ?>
            <div class="catalog-detail__cart js-replace-btns js-config-btns<?=$arBasketConfig['HTML'] ? '' : ' hidden'; ?> p-inline p-inline--16 pt pt--16" data-btn-config='<?=str_replace('\'', '"', CUtil::PhpToJSObject($arBtnConfig, false, true)); ?>'>
                <?=$arBasketConfig['HTML']; ?>
            </div>
        </div>
    <?endif; ?>
