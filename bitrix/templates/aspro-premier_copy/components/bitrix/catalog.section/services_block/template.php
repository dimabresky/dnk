...
$basketURL = (strlen(trim(TSolution::getFrontParametrValue('URL_BASKET_SECTION'))) ? trim(TSolution::getFrontParametrValue('URL_BASKET_SECTION')) : '');
...
$itemWrapperClasses = ['ui-card grid-list__item ui-card--image-scale stroke-theme-parent-all colored_theme_hover_bg-block color-theme-parent-all shadow-hovered shadow-no-border-hovered'];
if ($bBordered) {
...
?>
<?if (!$arParams['IS_AJAX']):?>
	<div class="services-list <?=$templateName?>-template">
					...
					<div class="p-block p-block--20 p-inline p-inline--24 flexbox">
						...
						<div class="pt pt--24 mt mt--auto flexbox flexbox--direction-column gap gap--12 flexbox--justify-between flex-grow-0">
							...
							<div class="order-info-btn line-block line-block--gap line-block--gap-12 line-block--align-normal"
								data-id="<?=$arItem['ID']?>"<?=($dataItem ? ' data-item="'.$dataItem.'"' : '');?>
							>
								<?if ($bOrderButton || $bCanBuy):?>
									...
									$arBtnConfig = [
										...
										'CONFIG' => array_merge(
											...
											[
												'EXPRESSION_ORDER_BUTTON' => TSolution::GetFrontParametrValue("EXPRESSION_SERVICE_ORDER_BUTTON") ?: Loc::getMessage('S_ORDER_SERVISE'),
