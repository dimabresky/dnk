...
if(!strlen($catalogPage)){
	...
	if(!$catalogIblockId){
		$catalogIblockId = \Bitrix\Main\Config\Option::get(
			'aspro.premier',
			'CATALOG_IBLOCK_ID',
			TSolution\Cache::$arIBlocks[SITE_ID]['aspro_premier_catalog']['aspro_premier_catalog'][0],
			SITE_ID
		);
	}
...
<div class="search-page-wrap">
	<form class="search " action="<?=($pathFile ? $pathFile : '')?>" method="get">
		<div class="">
			...
			<?else:?>
				<input class="form-control search-input" placeholder=" " type="text" name="q" value="<?=$arResult["REQUEST"]["QUERY"]?>" size="40" />
			<?endif;?>
			<?if($_REQUEST)
			{
		...
		<div class="search-button-div">
            <?TSolution\Functions::showBlockHtml([
                'FILE' => 'search/search-clear-button.php',
                'PARAMS' => [
                    'TITLE' => GetMessage('T_SEARCH_RESET'),
                ]
            ]);?>
	...
	<?
	// add search phrase on form submit - need for not authorized
	// sync by hash for authorized
	// TODO: show history
	?>
	<?\Aspro\Premier\SearchTitle::includeInputTools(
		'page',
		[
			'INPUT' => '.search-page-wrap [name="q"]',
			'MIN_QUERY_LEN' => 2,
		]
	);?>
