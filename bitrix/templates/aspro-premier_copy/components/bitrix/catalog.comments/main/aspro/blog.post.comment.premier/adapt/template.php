...
else
{
    ...
    else
    {
                ...
                <div class="blog-comment-fields">
                    ...
                    if($arResult["use_captcha"])
                    {
                        ...
                        <div class="captcha-row clearfix fill-animate">
                            <label class="font_13 color_666" for="captcha_word"><span><?=Loc::getMessage("B_B_MS_CAPTCHA_SYM")?>&nbsp;<span class="required-star">*</span></span></label>
                    ...
                    <div class="blog-comment-buttons-wrapper mt mt--32">
                        <input tabindex="10" class="btn btn-default btn-lg btn-wide" value="<?=GetMessage("B_B_MS_SEND")?>" type="button" name="sub-post" id="post-button" onclick="submitComment()">
                    </div>
                </div>
        ...
        $prevTab = 0;
        function ShowComment($comment, $tabCount=0, $tabSize=2, $canModerate=false, $User=Array(), $use_captcha=false, $bCanUserComment=false, $errorComment=false, $arParams = array())
        {
            ...
            if($comment["SHOW_AS_HIDDEN"] == "Y" || $comment["PUBLISH_STATUS"] == BLOG_PUBLISH_STATUS_PUBLISH || $comment["SHOW_SCREENNED"] == "Y" || $comment["ID"] == "preview")
            {
                ...
                if($tabCount <= 5)
                    $paddingSize = 2 * $tabCount;
                elseif($tabCount > 5 && $tabCount <= 10)
                    $paddingSize = 2 * 5 + ($tabCount - 5) * 1.5;
                elseif($tabCount > 10)
                    $paddingSize = 2 * 5 + 1.5 * 5 + ($tabCount-10) * 1;

                if(($tabCount+1) <= 5)
                    $paddingSizeNew = 2 * ($tabCount+1);
                elseif(($tabCount+1) > 5 && ($tabCount+1) <= 10)
                    $paddingSizeNew = 2 * 5 + (($tabCount+1) - 5) * 1.5;
                elseif(($tabCount+1) > 10)
                    $paddingSizeNew = 2 * 5 + 1.5 * 5 + (($tabCount+1)-10) * 1;
                $paddingSizeNew -= $paddingSize;

                if($prevTab > $tabCount)
                    $prevTab = $tabCount;
                if($prevTab <= 5)
                    $prevPaddingSize = 2 * $prevTab;
                elseif($prevTab > 5 && $prevTab <= 10)
                    $prevPaddingSize = 2 * 5 + ($prevTab - 5) * 1.5;
                elseif($prevTab > 10)
                    $prevPaddingSize = 2 * 5 + 1.5 * 5 + ($prevTab-10) * 1;

                    $prevTab = $tabCount;
                ?>
                <a name="<?=$comment["ID"]?>" id="c<?=$comment["ID"]?>"></a>
                <div class="blog-comment" style="padding-left:<?=$paddingSize?>rem;">
                ...
                if($comment["PUBLISH_STATUS"] == BLOG_PUBLISH_STATUS_PUBLISH || $comment["SHOW_SCREENNED"] == "Y" || $comment["ID"] == "preview")
                {
                    ...
                    <div class="blog-comment-info pb pb--8 pt pt--24 relative">
                        <?if($tabCount):?>
                            <div class="blog-comment-arrow"><?=TSolution::showSpriteIconSvg(SITE_TEMPLATE_PATH.'/images/svg/news/ui-elements.svg#comment-answer', '', ['WIDTH' => 12, 'HEIGHT' => 12]);?></div>
                        <?endif;?>
                        ...
                        {
                            ?>
                            <div class="blog-author font_15 color_dark fw-500">
                            <?
                        ...
                        {
                            ?>
                            <div class="blog-author font_15 color_dark fw-500"><?=$comment["AuthorName"]?></div>
                            <?
                        }
                        ...
                        ?>
                        <div class="blog-comment-date secondary-color font_13"><?=$comment["DateFormated"]?></div>
                    </div>
                    <div class="blog-clear-float"></div>
                    <div class="blog-comment-content font_15">
                        ...
                        ?>
                        <div class="blog-comment-meta font_13 line-block line-block--align-normal line-block--gap line-block--gap-24 line-block--flex-wrap line-block--row-gap line-block--row-gap-4">
                        <?
                        if ($arParams["SHOW_RATING"] == "Y")
                        {
                            ?>
                        ...
                        {
                            ?>
                            <span class="blog-comment-answer"><a class="color-theme-hover dotted" href="javascript:void(0)" onclick="return showComment('<?=$comment["ID"]?>')"><?=GetMessage("B_B_MS_REPLY")?></a></span>
                            <?
                        }
                        if(IntVal($comment["PARENT_ID"])>0)
                        {
                            ?>
                            <span class="blog-comment-parent"><a class="color-theme-hover dotted" href="#<?=$comment["PARENT_ID"]?>"><?=GetMessage("B_B_MS_PARENT")?></a></span>
                            <?
                        }
                        ?>
                        <span class="blog-comment-link"><a class="color-theme-hover dotted" href="#c<?=$comment["ID"]?>"><?=GetMessage("B_B_MS_LINK")?></a></span>
                        <?
                        if($comment["CAN_EDIT"] == "Y")
                        {
                            ...
                            <span class="blog-comment-edit"><a class="color-theme-hover dotted" href="javascript:void(0)" onclick="return editComment('<?=$comment["ID"]?>')"><?=GetMessage("BPC_MES_EDIT")?></a></span>
                            <?
                        }
                        if(strlen($comment["urlToShow"])>0)
                        {
                            ...
                            <span class="blog-comment-show">
                                <?if($arParams["AJAX_POST"] == "Y"):?>
                                    <a class="color-theme-hover dotted" href="javascript:void(0)" onclick="return hideShowComment('<?=$comment["urlToShow"]."&".bitrix_sessid_get()?>', '<?=$comment["ID"]?>');" title="<?=GetMessage("BPC_MES_SHOW")?>">
                                <?else:?>
                                    <a class="color-theme-hover dotted" href="<?=$comment["urlToShow"]."&".bitrix_sessid_get()?>" title="<?=GetMessage("BPC_MES_SHOW")?>">
                                <?endif;?>
                        ...
                        if(strlen($comment["urlToHide"])>0)
                        {
                            ...
                            <span class="blog-comment-show">
                                <?if($arParams["AJAX_POST"] == "Y"):?>
                                    <a class="color-theme-hover dotted" href="javascript:void(0)" onclick="return hideShowComment('<?=$comment["urlToHide"]."&".bitrix_sessid_get()?>&IBLOCK_ID=<?=htmlspecialcharsbx($_REQUEST["IBLOCK_ID"])?>&ELEMENT_ID=<?=htmlspecialcharsbx($_REQUEST["ELEMENT_ID"])?>', '<?=$comment["ID"]?>');" title="<?=GetMessage("BPC_MES_HIDE")?>">
                                <?else:?>
                                    <a class="color-theme-hover dotted" href="<?=$comment["urlToHide"]."&".bitrix_sessid_get()?>&IBLOCK_ID=<?=htmlspecialcharsbx($_REQUEST["IBLOCK_ID"])?>&ELEMENT_ID=<?=htmlspecialcharsbx($_REQUEST["ELEMENT_ID"])?>" title="<?=GetMessage("BPC_MES_HIDE")?>">
                                <?endif;?>
                        ...
                        if(strlen($comment["urlToDelete"])>0)
                        {
                            ...
                            <span class="blog-comment-delete">
                                <?if($arParams["AJAX_POST"] == "Y"):?>
                                    <a class="color-theme-hover dotted" href="javascript:void(0)" onclick="if(confirm('<?=GetMessage("BPC_MES_DELETE_POST_CONFIRM")?>')) deleteComment('<?=$comment["urlToDelete"]."&".bitrix_sessid_get()?>&IBLOCK_ID=<?=htmlspecialcharsbx($_REQUEST["IBLOCK_ID"])?>&ELEMENT_ID=<?=htmlspecialcharsbx($_REQUEST["ELEMENT_ID"])?>', '<?=$comment["ID"]?>');" title="<?=GetMessage("BPC_MES_DELETE")?>">
                                <?else:?>
                                    <a class="color-theme-hover dotted" href="javascript:if(confirm('<?=GetMessage("BPC_MES_DELETE_POST_CONFIRM")?>')) window.location='<?=$comment["urlToDelete"]."&".bitrix_sessid_get()?>&IBLOCK_ID=<?=htmlspecialcharsbx($_REQUEST["IBLOCK_ID"])?>&ELEMENT_ID=<?=htmlspecialcharsbx($_REQUEST["ELEMENT_ID"])?>'" title="<?=GetMessage("BPC_MES_DELETE")?>">
                                <?endif;?>
                        ...
                        {
                            ?>
                            <span class="blog-comment-delete blog-comment-spam"><a class="color-theme-hover dotted" href="<?=$comment["urlToSpam"]?>" title="<?=GetMessage("BPC_MES_SPAM_TITLE")?>"><?=GetMessage("BPC_MES_SPAM")?></a></span>
                            <?
                        }
                        ?>
                        </div>
                            ...
                            || (IntVal($_POST["edit_id"]) > 0 && IntVal($_POST["edit_id"]) == $comment["ID"] && $comment["CAN_EDIT"] == "Y")))
                    {
                        ...
                        ShowComment($commentPreview, (IntVal($_POST["edit_id"]) == $comment["ID"] && $comment["CAN_EDIT"] == "Y") ? $level : ($level+1), 2, false, Array(), false, false, false, $arParams);
                    }
        ...
        function RecursiveComments($sArray, $key, $level=0, $first=false, $canModerate=false, $User, $use_captcha, $bCanUserComment, $errorComment, $arSumComments, $arParams)
        {
            if(!empty($sArray[$key]))
            {
                foreach($sArray[$key] as $comment)
                {
                    ...
                    ShowComment($comment, $level, 2, $canModerate, $User, $use_captcha, $bCanUserComment, $errorComment, $arParams);
                    if(!empty($sArray[$comment["ID"]]))
                    {
                        foreach($sArray[$comment["ID"]] as $key1)
                        {
                            ...
                            ShowComment($key1, ($level+1), 2, $canModerate, $User, $use_captcha, $bCanUserComment, $errorComment, $arParams);
        ...
        if($arResult["is_ajax_post"] != "Y")
        {
            if($arResult["CanUserComment"])
            {
                ...
                ?>
                <div class="blog-add-comment mb mb--8"><a class="btn btn-secondary-black btn-lg" href="javascript:void(0)"><?=GetMessage("B_B_MS_ADD_COMMENT")?></a></div>
                <a name="0"></a>
                <?
