...
/**
 * Persistence for sticker assignments (element + sticker_xml_id + assigned_at).
 */
final class AssignmentTracker
{
    ...
    /**
     * Creates assignment if missing. Does not overwrite existing ASSIGNED_AT.
     *
    ...
    public static function trackIfMissing(
        ...
        $result = AssignmentTable::add([
            ...
            'ELEMENT_ID' => $elementId,
            'STICKER_XML_ID' => $stickerXmlId,
            'ASSIGNED_AT' => $assignedAt ?? new DateTime(),
     ...
     */
    public static function findExpired(string $stickerXmlId, float $lifetimeDays, int $limit = 100): array
    {
        $stickerXmlId = strtoupper(trim($stickerXmlId));
        ...
        $threshold = Config::lifetimeThreshold(new \DateTimeImmutable('now'), $lifetimeDays);

        $rows = AssignmentTable::getList([
            'filter' => [
                '=STICKER_XML_ID' => $stickerXmlId,
                '<=ASSIGNED_AT' => $threshold,
            ],
            'order' => ['ID' => 'ASC'],
            'limit' => $limit,
        ])->fetchAll();
