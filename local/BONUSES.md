...
Дата списания принимается в формате `дд.мм.гггг`; пустое значение очищает `UF_BONUS_EXPIRE_DATE`. Невалидная непустая дата пишется в лог `[invalid_expire_date]`.
