 ...
 */
final class ImshopCatalogFeedExporter
{
    ...
    public static function export(
        ...
        $navChainCache = [];
        $offerCount = 0;
        $currency = self::resolveBaseCurrency();
        ...
        try {
            self::writeFeedHeader($targetFp, $siteUrl, $shopName, $currency);
    ...
    private static function buildOfferXml(
        ...
        $lines[] = '        <currencyId>' . self::escapeXml($currency) . '</currencyId>';

        foreach ($categoryIds as $categoryId) {
            $lines[] = '        <categoryId>' . $categoryId . '</categoryId>';
        }

        foreach (self::resolvePictureUrls($fields, $props, $siteUrl) as $picture) {
        ...
        $barcode = self::resolveBarcode($props);
        if ($barcode !== '') {
        ...
        $country = self::firstPropertyValue($props['STRANA_IZGOTOVLENIYA'] ?? null);
        if ($country !== '') {
        ...
        $video = self::resolveVideoUrl($props);
        if ($video !== '') {
     ...
     * @return list<string>
     */
    private static function resolvePictureUrls(array $fields, array $props, string $siteUrl): array
    {
        $urls = [];
        $usedFileIds = [];

        $detailPictureId = (int) ($fields['DETAIL_PICTURE'] ?? 0);
        $mainPictureId = self::resolveFreshFeedPictureFileId($props, $detailPictureId);
        if ($mainPictureId <= 0) {
            $mainPictureId = $detailPictureId;
        }
        if ($mainPictureId <= 0) {
            $mainPictureId = (int) ($fields['PREVIEW_PICTURE'] ?? 0);
        }
        if ($mainPictureId > 0) {
            $url = self::fileIdToUrl($mainPictureId, $siteUrl);
            if ($url !== '') {
                $urls[] = $url;
                $usedFileIds[$mainPictureId] = true;
            }
        }

        foreach (self::propertyFileIds($props['MORE_PHOTO'] ?? null) as $fileId) {
            if (isset($usedFileIds[$fileId])) {
                continue;
            }
            $url = self::fileIdToUrl($fileId, $siteUrl);
            if ($url === '') {
                continue;
            }
            $urls[] = $url;
     ...
     * @param array<string, mixed> $props
     */
    private static function resolveBarcode(array $props): string
    {
     ...
     * @param array<string, mixed> $props
     */
    private static function resolveVideoUrl(array $props): string
    {
    ...
    private static function resolveBaseCurrency(): string
    {
    ...
    private static function fileIdToUrl(int $fileId, string $siteUrl): string
    {
        if ($fileId <= 0) {
            return '';
        }
     ...
     * @return list<int>
     */
    private static function propertyFileIds(?array $property): array
    {
        if ($property === null) {
            return [];
        }
     ...
     * @param resource $fp
     */
    private static function writeFeedHeader($fp, string $siteUrl, string $shopName, string $currency): void
    {
        ...
        fwrite($fp, '      <currency id="' . self::escapeXml($currency) . '" rate="1"/>' . "\n");
        fwrite($fp, "    </currencies>\n");
    }
