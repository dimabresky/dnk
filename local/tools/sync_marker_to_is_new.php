...
/**
 * Массовая синхронизация свойства IS_NEW из MARKER_DLYA_SAYTA для каталога DNK_CATALOG_IBLOCK_ID.
 * Запуск из корня сайта: php local/tools/sync_marker_to_is_new.php
 */
...
while ($row = $res->Fetch()) {
    ...
    ++$processed;
    if (IblockProductMarkerIsNewEvents::syncIsNewFromMarkerForElement((int) DNK_CATALOG_IBLOCK_ID, $id)) {
        ++$updated;
    }
}

echo "Processed elements: {$processed}\n";
echo "IS_NEW synced: {$updated}\n";
